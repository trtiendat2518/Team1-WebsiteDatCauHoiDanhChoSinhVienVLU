...
<div class="grid grid-3-6-3 mobile-prefer-content">
	...
	<div class="grid-column">
		...
		<br>
		@foreach ($postpin as $key => $pin)
		<div class="widget-box no-padding optionsocial">
			@php
			if(Session::get('student_email')==$pin->student->student_email){
				@endphp
				<div class="widget-box-settings">
					<div class="post-settings-wrap">
						...
						<div class="simple-dropdown widget-box-post-settings-dropdown">
							<a href="javascript:void(0)" type="button" class="postD simple-dropdown-link" id="postD" data-id_post="{{$pin->post_id}}">Xóa câu hỏi</a>
						</div>
					</div>
				</div>
				@php
				...
				<div class="widget-box-status-content">
					<div class="user-status">
						<a class="user-status-avatar">
							<div class="user-avatar small no-outline">
								<div class="user-avatar-content">
									@if ($pin->student->student_info_id)
									<div class="hexagon-image-30-32" data-src="{{asset('public/student/img/avatar/'.$pin->student->student_avatar)}}"></div>
									@else
					...
					<div class="tag-list">
						<a class="tag-item secondary" style="font-size: 16px" href="{{url('/cau-hoi-theo-loai/'.$pin->category_id)}}">{{$pin->category->category_name}}</a>
					</div>
					...
					<div class="content-actions">
						<div class="content-action">
							<div class="meta-line">
								...
								<p class="meta-line-text likelike">{{$pin->post_like}}</p>
							</div>
						</div>
						<div class="content-action">
							<div class="meta-line">
								<p class="meta-line-link">{{$pin->comments()->count()}} Lượt bình luận</p>
							</div>
							@if ($pin->post_reply=='')
							<div class="meta-line">
			...
			<div class="post-options">
				@php
				if ($pin->likes->contains('student_id',Session::get('student_id')) && $pin->likes->contains('post_id',$pin->post_id) && $pin->likes->contains('like_quantity',1)){
					@endphp
					<div class="post-option-wrap postL" data-id_like="{{$pin->post_id}}">
				...
				@else
				<div class="post-option popup-event-creation-trigger" onclick="getPost({{$pin->post_id}})">
					<svg class="post-option-icon icon-share">
						<use xlink:href="#svg-quests"></use>
					</svg>
			...
			<!-- POST COMMENT LIST -->
			<div id="commentId_{{$pin->post_id}}" style="display: none;"  class="post-comment-list ">
							...
							<div class="form-row">
								<div class="form-item">
									<div class="form-input small">
										...
										<textarea style="height: 0%; width: 70%; margin-top: 20px; margin-left: 80px;" class="cmtcontent_{{$pin->post_id}}" name="comment_content" rows="2" id="post-reply"></textarea>
										<button style="margin-top: 60px; line-height: 0px; height: 30px; margin-right: 10px" type="button" data-id_post="{{$pin->post_id}}" class="postC button secondary">Gửi</button>
									</div>
								</div>
							</div>
						</form>
				...
				<div id="all_cmt">
					<input type="hidden" name="postId" class="postId" value="{{$pin->post_id}}">
					@foreach ($pin->comments as $key => $cmt)
					<div style="display: none;" class="post-comment lineCmt_{{$pin->post_id}}" id="lineCmt_{{$pin->post_id}}">
				...
				@if ($pin->comments()->count() > 3)
				<p class="post-comment-heading loadM_{{$pin->post_id}}">Xem thêm...</p>
				@endif
			</div>
			<!-- /POST COMMENT LIST -->
		</div>
